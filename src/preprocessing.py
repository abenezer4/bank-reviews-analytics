...
class ReviewPreprocessor:
    ...
    def drop_amharic_reviews(self):
        """Drop reviews containing Amharic content"""
        print("\n[Amharic Filter] Dropping reviews with Amharic text...")
        amharic_pattern = re.compile(r'[\u1200-\u137F]+')
        before_count = len(self.df)
        # Keep only reviews that do NOT contain Amharic characters
        self.df = self.df[~self.df['review_text'].apply(lambda x: bool(amharic_pattern.search(str(x))))]
        removed = before_count - len(self.df)
        print(f"Removed {removed} reviews containing Amharic text")
